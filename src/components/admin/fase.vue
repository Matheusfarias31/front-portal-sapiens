<template>
  <v-sheet class="py-5 px-10">
    <v-data-table :headers="headerFase" :items="dessertsFase" :search="search" :loading="loadingTable"
      ...
      <template v-slot:top>
        <v-toolbar flat class="mb-8 rounded" dark color="deep-purple lighten-2">
          <v-toolbar-title>Fases</v-toolbar-title>
              ...
              <v-card>
                ...
                <v-card-text>
                  <v-container>
                    <v-row>
                      <v-col cols="12">
                        <v-text-field v-model="editedItem.NomeFase" label="Nome Fase" required></v-text-field>
                      </v-col>
                    </v-row>
                  </v-container>
                </v-card-text>
...
export default {
  ...
  methods: {
    initialize() {
      ...
      axios({
        method: "get",
        url: `${process.env.VUE_APP_ROOT_API_BASE_URL}fases`,
      })
        .then((result) => {
          this.loadingTable = false;
          this.dessertsFase = result.data.result;
        })
        .catch((err) => {
          console.log(err);
        });
